...
import andbur.hub.top.uma.DNodeSet.DNodeSetElement;
import andbur.hub.top.uma.InvalidModelException;
...
//				name = "CUT(" + name + ")";
		...
		for (DNode n : allNodes) {
			if (n.isEvent) {
				...
				for (DNode pre : n.pre) {
					unfolding.addArc((Transition) nodeMap.get(pre.globalId), (Place) nodeMap.get(n.globalId));
				}
			}
		}
